...
class Capybara::Capachrome::Driver < Capybara::Driver::Base
  DEFAULT_OPTIONS = {
    :browser => :firefox
  }
  ...
  def browser
    unless @browser
      @browser = Capachrome::WebDriver::Driver.for(options[:browser], options.reject { |key,val| SPECIAL_OPTIONS.include?(key) })
